...
Logit.ProfilePage = {
  ...
  async init() {
    ...
    try {
      ...
      if (typeof Logit.PosterCycle !== 'undefined' && this._movies)
        Logit.PosterCycle.start(this._movies, false);
      this.loadStats();
  ...
  async loadStats() {
    ...
    var ratedMovies = movies.filter(function (m) {
      return Number(m.r) > 0;
    });
    var avgRating =
      ratedMovies.length > 0
        ? (
            ratedMovies.reduce(function (a, b) {
              return a + Number(b.r);
    ...
    var rewatchCount = movies.filter(function (m) {
      return Logit.Utils.isRewatch(m);
    }).length;
    var rewatchEl = document.getElementById('statRewatch');
    if (rewatchEl) rewatchEl.textContent = rewatchCount;

    var totalMins = movies.reduce(function (a, b) {
      return a + (Number(b.rt) || 0);
    }, 0);
  ...
  loadFavorites() {
    ...
    for (var i = 0; i < 4; i++) {
      if (favs[i]) {
        ...
        card.innerHTML =
          '<img src="' +
          Logit.Utils.esc(favs[i].poster || '') +
          '" alt="' +
          Logit.Utils.esc(favs[i].t || '') +
          '">' +
          '<button class="favRemove" data-index="' +
          i +
          '">&times;</button>';
        ...
        card.querySelector('.favRemove').addEventListener(
          'click',
          (function (idx) {
            return function (e) {
              e.stopPropagation();
              self.removeFavorite(idx);
            };
          })(i)
        );
        ...
        placeholder.innerHTML =
          '<svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg><span>Add</span>';
        placeholder.addEventListener('click', function () {
          self._editingFavIndex = null;
          self.openFavModal();
        });
    ...
    if (modal) {
      ...
      if (confirmBtn) {
        ...
        confirmBtn.textContent =
          this._editingFavIndex !== null ? 'Change Poster' : 'Add to Favorites';
      }
    }
  },
  ...
  async searchFavMovies(query) {
    ...
    try {
      var data = await Logit.Search.tmdb(
        ...
        item.addEventListener('click', function () {
          self._selectedFav = {
            ...
            poster: item.dataset.poster
              ? 'https://image.tmdb.org/t/p/w342' + item.dataset.poster
              : ''
          };
  ...
  async syncAvatarToCloud(avatarData) {
    ...
    var userId = Logit.Auth.getUserId();
    if (!client || !userId) {
      console.warn('Avatar sync: no client or userId');
      return;
    }
    try {
  ...
  setupListeners() {
    ...
    if ($('clearAvatarBtn'))
      $('clearAvatarBtn').addEventListener('click', function () {
        ...
        var user = self._user;
        var username =
          (user && user.user_metadata && user.user_metadata.username) || 'User';
      ...
      $('editUsernameBtn').addEventListener('click', async function () {
        ...
        if (client && self._user) {
          ...
          await client
            .from('users')
            .upsert(
              { id: self._user.id, email: self._user.email, username: newName },
              { onConflict: 'id' }
            );
        }
    ...
    if ($('driveBackupBtn'))
      $('driveBackupBtn').addEventListener('click', async function () {
        var btn = $('driveBackupBtn');
        var titleEl =
          btn.querySelector('.driveActionTitle') || btn.querySelector('span');
        ...
        try {
          Logit.Drive.init();
          await new Promise(function (resolve, reject) {
            Logit.Drive.requestAuth(resolve, reject);
          });
          await updateDriveStatusUI();
    ...
    if ($('driveRestoreBtn'))
      $('driveRestoreBtn').addEventListener('click', async function () {
        if (
          !confirm(
            'Restore will add movies from your Google Drive backup. Continue?'
          )
        )
          return;
    ...
    if ($('exportJsonBtn'))
      $('exportJsonBtn').addEventListener('click', async function () {
        var result = await Logit.Storage.loadMovies();
        Logit.Export.doExport(result.movies, 'json', function () {
    ...
    if ($('exportTxtBtn'))
      $('exportTxtBtn').addEventListener('click', async function () {
        var result = await Logit.Storage.loadMovies();
        Logit.Export.doExport(result.movies, 'txt', function () {
          Logit.Utils.closeModal($('exportModal'));
        });
      });
    if ($('exportCancelBtn'))
    ...
    if ($('importStartBtn'))
      $('importStartBtn').onclick = async function () {
        ...
        if (text.trim().charAt(0) === '[' || text.trim().charAt(0) === '{') {
          try {
            ...
            if (Logit.Import.isSlimExport(arr)) {
              if (!API) {
                if (statusEl) statusEl.textContent = 'TMDB API key required.';
                return;
              }
              btn.disabled = true;
              var existingTmdbIds = new Set(
              ...
              for (var i = 0; i < arr.length; i++) {
                ...
                if (statusEl)
                  statusEl.textContent =
                    'Fetching ' + (i + 1) + '/' + arr.length;
                try {
                  var detail = await Logit.Search.tmdb(
                  ...
                  var newMovie = Logit.MovieFactory.fromTMDB(
                    detail,
                    entry.r || 3,
                    entry.w || '1st Watch',
        ...
        var imported2 = 0,
          failed2 = 0,
          skipped = 0;
        var existingTmdbIds3 = new Set(
          movies
            .filter(function (m) {
              return m.tmdb_id;
            })
            .map(function (m) {
              return String(m.tmdb_id);
            })
        );
        for (var k = 0; k < lines.length; k++) {
          ...
          if (statusEl)
            statusEl.textContent = 'Importing ' + (k + 1) + '/' + lines.length;
          try {
            var tmdbId = entry2.tmdbId || '';
            if (!tmdbId && entry2.imdbId) {
              var fd = await Logit.Search.tmdb(
              ...
              if (fd && fd.movie_results && fd.movie_results.length > 0)
                tmdbId = String(fd.movie_results[0].id);
            }
            if (!tmdbId && entry2.title) {
              var sUrl =
                ...
                API +
                '&query=' +
                encodeURIComponent(entry2.title);
              if (entry2.year) sUrl += '&year=' + entry2.year;
              var sd = await Logit.Search.tmdb(sUrl);
              if (sd && sd.results && sd.results.length > 0)
                tmdbId = String(sd.results[0].id);
            }
            if (!tmdbId) {
              failed2++;
              continue;
            }
            if (existingTmdbIds3.has(tmdbId)) {
              skipped++;
              continue;
            }
            var detail2 = await Logit.Search.tmdb(
              'https://api.themoviedb.org/3/movie/' +
                tmdbId +
                '?api_key=' +
                API +
            ...
            var watch = entry2.rewatch
              ? 'Rewatch'
              : Logit.Movies.watchType(movies, detail2.title || '');
            var newMovie2 = Logit.MovieFactory.fromTMDB(
              detail2,
              entry2.rating || 3,
              watch,
              Logit.Import.normalizeDate(entry2.date)
            );
    ...
    if ($('modalChangePasswordStartBtn'))
      $('modalChangePasswordStartBtn').addEventListener(
        'click',
        async function () {
          ...
          if (!newPass || newPass.length < 6) {
            statusEl.textContent = 'New password must be 6+ characters.';
            return;
          }
          ...
          var client = Logit.Supabase.getClient();
          if (!client) {
            statusEl.textContent = 'Database client not connected.';
          ...
          if (!email) {
            statusEl.textContent = 'No email found. Are you logged in?';
            statusEl.style.color = 'var(--red)';
            return;
          }

          var { error: loginError } = await client.auth.signInWithPassword({
            email: email,
    ...
    if ($('modalForgotPasswordBtn'))
      $('modalForgotPasswordBtn').addEventListener('click', async function () {
        var statusEl = $('changePasswordStatus');
        ...
        if (error) {
          ...
          statusEl.textContent = 'Reset link sent! Please check your inbox.';
          statusEl.style.color = 'var(--green)';
        }
      });
    ...
    if ($('favRemoveSelect'))
      $('favRemoveSelect').addEventListener('click', function () {
  ...
  async searchDirectors(query) {
    ...
    try {
      ...
      var directors = data.results.filter(function (p) {
        return p.known_for_department === 'Directing' && p.profile_path;
      });
      var fragment = document.createDocumentFragment();
      for (var i = 0; i < Math.min(directors.length, 8); i++) {
        ...
        item.innerHTML =
          ...
          imgUrl +
          '" alt="' +
          Logit.Utils.esc(p.name) +
          '"><div class="directorItemInfo"><div class="directorItemName">' +
          Logit.Utils.esc(p.name) +
          '</div><div class="directorItemKnown">Director</div></div>';
        fragment.append(item);
      }
      results.textContent = '';
  ...
  async deleteAccount() {
    if (
      !confirm(
        'Delete your account? Your data will be kept for 30 days for recovery.'
      )
    )
      return;
    if (
      !confirm('After 30 days, all data will be permanently deleted. Continue?')
    )
      return;
    try {
      var client = Logit.Supabase.getClient();
