...
(async function backfill() {
  ...
  if (!API) {
    console.error('No TMDB API key found. Set it from About page.');
    return;
  }
  ...
  var { data: movies, error } = await client
    .from('movies')
    .select('*')
    .eq('user_id', userId);
  if (error) {
  ...
  var toUpdate = movies.filter(function (m) {
    return !m.sc && !m.pc && m.tmdb_id;
  });
  console.log(
    'Found ' +
      movies.length +
  ...
  for (var i = 0; i < toUpdate.length; i++) {
    ...
    try {
      ...
      var prods = (d.production_companies || [])
        .map(function (x) {
          return x.name;
        })
        .filter(function (n) {
